 ...
 * 刷新布局与内容布局同步下拉
 */
public class ContentAndRefreshDownSlider implements CustomSwipeRefreshLayout.Slider {
    ...
    public void onLayout(int left, int right, int top, int bottom) {
        ...
        mRefreshView.layout(left + layoutParams1.leftMargin, top - height, left + measuredWidth + layoutParams1.leftMargin, top);
    }
    ...
    public void scroll(float y) {
        ...
        if (y < 0) {
            mDistance = 0;
            mParentVeiw.scrollTo(0,mDistance);

        } else if (y > measuredHeight) {
            mDistance = measuredHeight;
            mParentVeiw.scrollTo((int) mParentVeiw.getTranslationX(),-mDistance);
